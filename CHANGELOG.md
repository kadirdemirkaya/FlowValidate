...
- `StopOnFirstFailure(bool enabled = true)` on the `RuleFor` chain and `UseStopOnFirstFailure(bool enabled = true)` on `BaseValidator<T>`: opt in to stopping a chain at its first failing rule instead of running every rule and reporting every failure (`RuleFor(x => x.Email).IsNotEmpty().IsEmail()` against `Email = ""` now reports only `NotEmpty` when opted in, instead of `NotEmpty` and `Email`). `StopOnFirstFailure` applies to the whole chain regardless of where it is written, the same way `WithDescriptiveMessages` does, and overrides the validator-wide `UseStopOnFirstFailure` default in either direction. A failing `Should`/`ShouldAsync` rule still reports every error it raised before the chain stops, and an async rule after the failing one is skipped without being awaited. `RequiredIf`'s existing `SkipRemainingRules` behavior is unchanged. Off by default: without either call every chain still runs exactly as before.
- Both `FlowValidate` and `FlowValidate.AspNetCore` NuGet packages now carry a package icon, and `README.md` shows the same logo at the top.
